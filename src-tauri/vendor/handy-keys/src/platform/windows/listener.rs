    ...
    HOOK_CONTEXT.with(|ctx_cell| {
        ...
        if let Some(ctx) = ctx_ref.as_mut() {
            ...
            if let Some(modifier) = vk_to_modifier(vk_code) {
                ...
                // Blocking is decided for EVERY modifier transition (not just
                // state changes, so autorepeat of a held hotkey modifier stays
                // blocked) and never for releases — swallowing a release would
                // leave that modifier stuck down at the OS level (keyboard
                // hijack). See `should_block_modifier_event`.
                should_block = crate::platform::state::should_block_modifier_event(
